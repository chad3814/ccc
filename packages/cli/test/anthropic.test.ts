...
describe('AnthropicGenerator', () => {
  it('sends the request shape ccc relies on', async () => {
    ...
    const turn = await generator.start({ system: 'sys' }).send('write it', 'claude-opus-5');
  ...
  it('answers the previous tool call with an error result when sending feedback', async () => {
    ...
    const session = generator.start({ system: 's' });
    await session.send('first', 'claude-opus-5');
    const second = await session.send('fix it', 'claude-opus-5');
  ...
  it('treats a reply without a tool call as a failed turn and continues with plain text', async () => {
    ...
    const session = generator.start({ system: 's' });
    expect(await session.send('go', 'claude-opus-5')).toMatchObject({ code: null, note: 'you did not call write_module' });
    await session.send('call the tool', 'claude-opus-5');
  ...
  it('reports refusals and truncation, and still closes a truncated tool call', async () => {
    ...
    expect(await refusal.generator.start({ system: 's' }).send('go', 'claude-opus-5')).toMatchObject({
      code: null,
    ...
    const session = cut.generator.start({ system: 's' });
    expect((await session.send('go', 'claude-opus-5')).note).toBe('the reply hit max_tokens (64000) and was cut off; write a shorter module');
    await session.send('shorter please', 'claude-opus-5');
...
describe('escalation within a session', () => {
  it('sends each turn to the model it names, with that model\'s settings, keeping the conversation', async () => {
    const { params, generator } = scripted([reply([writeCall('t1', 'a')]), reply([writeCall('t2', 'b')])]);
    const session = generator.start({ system: 's' });
    await session.send('first', 'claude-haiku-4-5');
    await session.send('fix it', 'claude-opus-5');
    expect(params[0]).toMatchObject({ model: 'claude-haiku-4-5', thinking: { type: 'enabled', budget_tokens: 16_000 } });
    expect(params[0]?.fallbacks).toBeUndefined();
    expect(params[1]).toMatchObject({ model: 'claude-opus-5', thinking: { type: 'adaptive' }, fallbacks: 'default' });
    expect(params[1]?.messages).toHaveLength(3);
  });

  it('names the model of the failing turn in GeneratorUnavailable', async () => {
    const missing = new Anthropic.NotFoundError(404, { type: 'error' }, 'model not found', new Headers());
    const generator = new AnthropicGenerator(async (request) => {
      if (request.model === 'claude-opus-5') throw missing;
      return reply([writeCall('t1', 'a')]);
    });
    const session = generator.start({ system: 's' });
    await session.send('first', 'claude-haiku-4-5');
    await expect(session.send('fix it', 'claude-opus-5')).rejects.toThrow("model 'claude-opus-5' is not available to this API key");
  });
});
...
describe('model-aware requests', () => {
  ...
  it('uses adaptive thinking and server-side fallbacks on current frontier models', () => {
    const request = buildRequest('claude-opus-5', 's', messages);
  ...
  it('uses adaptive thinking without fallbacks on Sonnet 5', () => {
    const request = buildRequest('claude-sonnet-5', 's', messages);
    expect(request.thinking).toEqual({ type: 'adaptive' });
  ...
  it('uses a thinking budget on Haiku 4.5', () => {
    const request = buildRequest('claude-haiku-4-5', 's', messages);
  ...
  it('sends no thinking or fallback settings for models it does not know', () => {
    const request = buildRequest('some-future-model', 's', messages);
...
describe('unavailable generators', () => {
  ...
  it('turns a persistent 429 into GeneratorUnavailable, noting missing rate-limit headers', async () => {
    ...
    const send = failing(error).start({ system: 's' }).send('go', 'claude-opus-5');
  ...
  it('reports ordinary rate limits with the limit that was hit', async () => {
    ...
    await expect(failing(error).start({ system: 's' }).send('go', 'claude-opus-5')).rejects.toThrow(
  ...
  it('turns authentication and missing-model errors into GeneratorUnavailable', async () => {
    ...
    await expect(failing(auth).start({ system: 's' }).send('go', 'claude-opus-5')).rejects.toThrow(
      'authentication failed; check ANTHROPIC_API_KEY (or your `ant auth login` profile)',
    );
    const missing = new Anthropic.NotFoundError(404, { type: 'error' }, 'model not found', new Headers());
    await expect(failing(missing).start({ system: 's' }).send('go', 'claude-nope')).rejects.toThrow(
