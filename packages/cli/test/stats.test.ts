...
describe('stats', () => {
  it('summarizes generation history', () => {
    ...
    hand.history.push(gen('tests', 1, 'passed', 0.01), { ...gen('impl', 2, 'passed', 0.03), escalations: 1 }, gen('impl', 3, 'failed', 0.05));
    hand.testFileHash = 'x';
    ...
    expect(stats.impl).toEqual({ generations: 3, firstAttemptPasses: 1, passed: 2, meanAttempts: 2, costUsd: 0.1, unpriced: 0, escalated: 1 });
    expect(stats.tests).toEqual({ generations: 2, firstAttemptPasses: 2, passed: 2, meanAttempts: 1, costUsd: 0.01, unpriced: 1, escalated: 0 });
    ...
    expect(formatStats(stats)).toBe(
      [
        'implementations: 3 generation(s), 1 passed on the first attempt (33%), 2 passed, mean 2.00 attempts, 1 escalated, $0.1000',
