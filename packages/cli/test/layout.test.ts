...
describe('paths', () => {
  it('maps concept ids to generated file paths', () => {
    ...
    expect(contractPath('game.hand')).toBe('.ccc/gen/game/hand.contract.d.ts');
...
describe('sources', () => {
  ...
  it('asserts type exports are identical in both directions', () => {
    const source = conformanceSource('card', ['Card', 'Suit']);
    ...
    expect(source).toContain("export const type_Suit: Equal<impl.Suit, iface.Suit> extends true ? true : 'Suit differs from the interface' = true;");
  });
