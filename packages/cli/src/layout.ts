...
// nominally.
//
// Type-only exports (interfaces, non-generic aliases) get a two-way identity
// check, since `typeof` sees values only.
export function conformanceSource(id: ConceptId, types: readonly string[]): string {
  ...
  const typeChecks =
    types.length === 0
      ...
      : [
          ...
          ...types.map(
            (name) =>
              `export const type_${name}: Equal<impl.${name}, iface.${name}> extends true ? true : '${name} differs from the interface' = true;`,
          ),
        ];
  return [
