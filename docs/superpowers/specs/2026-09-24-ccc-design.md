...
The model returns code through a tool call `write_module({ code: string })` (strict schema, `tool_choice: auto`). Requests stream with `max_tokens: 64000`, adaptive thinking, and server-side refusal fallbacks (`betas: ['server-side-fallback-2026-07-01']`, `fallbacks: 'default'`). A reply with no tool call, a truncated call, or a refusal counts as a failed attempt. ccc prepends a deterministic header:
